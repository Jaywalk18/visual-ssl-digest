...
![](images/180a0a03357e518c70e62fb14f17a64c204953b9f95ae0bbeda22edf2f510edd.jpg)

<details>
<summary>line chart</summary>

| Training Steps | LingBot-VA | Next Forcing (Ours) |
| -------------- | ---------- | ------------------- |
| 10k            | 75         | 85                  |
| 20k            | 90         | 92                  |
| 30k            | 92         | 94                  |
...
![](images/cbc305ae7d2e7b6cccf4c511c4943b5e96cf25de9c0eb1f9b1f968dbc6a8cf93.jpg)

<details>
<summary>line chart</summary>

| Training Steps | Series 1 | Series 2 |
| -------------- | -------- | -------- |
| 10k            | 80       | 74       |
| 20k            | 88       | 86       |
| 30k            | 90       | 89       |
...
![](images/9af5f6589e2dd01097324a5a56429e83cb5c6126d8844c9ceed476bf7babb732.jpg)

<details>
<summary>line chart</summary>

| Training Steps | Clean | Unclean |
| -------------- | ----- | ------- |
| 10k            | 70    | 65      |
| 20k            | 85    | 80      |
| 30k            | 90    | 85      |
...
![](images/15efa6ba36afd2b422036a1b3f80893cb76431218bdffd46dd94becfb558deb0.jpg)

<details>
<summary>line chart</summary>

| Training Steps | Blue Line Value | Red Line Value |
| -------------- | --------------- | -------------- |
| 10k            | 29.7            | +29.7          |
| 20k            | -               | -              |
| 30k            | -               | -              |
...
![](images/884d17747454cc9fce58dcd85042d4798e7c260343dcd7a5d97dac4aefb0341e.jpg)
...
```mermaid
graph TD
  A["Main Model"] --> B["Transformer Block x 30"]
  B --> C["Multi-Layer Fusion (MLP)"]
  C --> D["MCP Module 1"]
  D --> E["Transformer Block x 3"]
  E --> F["Linear Proj"]
  F --> G["Concat"]
  G --> H["Embed"]
  H --> I["Noisy next¹ chunk"]
  D --> J["Transformer Block x 3"]
  J --> K["Linear Proj"]
  K --> L["Concat"]
  L --> M["Embed"]
  M --> N["Noisy next² chunk"]
  D --> O["Transformer Block x 3"]
  O --> P["Linear Proj"]
  P --> Q["Concat"]
  Q --> R["Embed"]
  R --> S["Noisy next³ chunk"]
...
We build upon the autoregressive video-action framework of LingBot-VA [36]. The model operates on video latents encoded by a pre-trained VAE and generates video frames in chunks of M frames. $\mathbf { x } _ { t } ^ { ( i ) }$ conditioned on the $\mathbf { x } _ { 0 } ^ { ( 1 : \bar { i } - 1 ) }$

$$
v _ {\theta} \left(\mathbf {x} _ {t} ^ {(i)}, t, \left[ \mathbf {x} _ {0} ^ {(1: i - 1)}, \ell \right]\right) \approx \boldsymbol {\epsilon} ^ {(i)} - \mathbf {x} _ {0} ^ {(i)}. \tag {3}
$$
...
$$
\mathrm{RoPE} (\mathbf {x} _ {0} ^ {[ k ]} [ i ]) = \mathrm{RoPE} (i + k), \tag {6}
$$
...
$$
\mathbf {h} _ {\text { fuse }} = \mathrm{MLP} ([ \mathbf {h} _ {4}; \mathbf {h} _ {1 2}; \mathbf {h} _ {2 0}; \mathbf {h} _ {3 0} ]) \in \mathbb {R} ^ {B \times N \times d}, \tag {7}
$$
...
Causal Chain Across Depths. The three MCP modules form a causal chain. For each depth k, the noisy shifted target x[k]tk $\mathbf { x } _ { t _ { k } } ^ { [ k ] }$ is embedded through the shared patch embedding layer and fused with the output from the previous depth:
...
where $\mathbf { x } _ { \leq i }$ denotes the observation history up to chunk $i , \mathbf { a } _ { < i }$ denotes the action history, and $\ell$ is the language instruction. The video stream first predicts the next visual state $\mathbf x _ { i + 1 }$ , then the action stream decodes ${ \bf a } _ { i }$ by conditioning on the observations including the predicted future chunk $\mathbf x _ { i + 1 }$ .
...
$$
\mathcal {L} _ {\text { video }} = \mathbb {E} _ {t, \mathbf {x} _ {0}, \boldsymbol {\epsilon}} \left[ \left\| v _ {\theta} (\mathbf {x} _ {t}, t, \mathbf {c}) - (\boldsymbol {\epsilon} - \mathbf {x} _ {0}) \right\| ^ {2} \right], \tag {10}
$$

and the action loss supervises the action stream via inverse dynamics:

$$
\mathcal {L} _ {\text { action }} = \mathbb {E} _ {t, \mathbf {a} _ {0}, \epsilon} \left[ \left\| v _ {\psi} (\mathbf {a} _ {t}, t, \mathbf {c} _ {a}) - (\epsilon - \mathbf {a} _ {0}) \right\| ^ {2} \right], \tag {11}
$$
...
![](images/7a3ae35734071f97de74f5a84818baa902d80c91fa65d1ee0775532421ee8add.jpg)
...
graph TD
  ...
  Q1 --> R1["Blue Box"]
    ...
    style Ground truth fill:#f9f,stroke:#333
    style Ours fill:#ccf,stroke:#333
    style Baseline fill:#cfc,stroke:#333
```
</details>
...
![](images/4bae3d1cc327512278a369ec852b3543331a190e6fa6e7aeb25590839a99380b.jpg)

![](images/a7e59ad865cd04f4a4c25e3845fb080b35c799b83dc3c3fa19cb4bf870879585.jpg)

<details>
<summary>line chart</summary>

| Training Steps | FVD (↓) - Blue Line | FVD (↓) - Red Line |
| -------------- | ------------------- | ------------------ |
| 10k            | 350                 | 140                |
| 20k            | 300                 | 110                |
| 30k            | 290                 | 100                |
...
![](images/8dc596e76c4cb636308210cae103a1214c506a18215906378232f19806fdc482.jpg)

<details>
<summary>line chart</summary>

| Training Steps | Blue Line | Red Line |
| -------------- | --------- | -------- |
| 10k            | 220       | 125      |
| 20k            | 215       | 110      |
| 30k            | 225       | 115      |
...
![](images/0ecd70bef6879d2f43e7691f2bfc4d5b128f592c9cac329ee27a022b6594802c.jpg)
...
![](images/3b5723efaf110e3a51d01353618170f4cd6566c3a08ff9065b95db96ff08ba65.jpg)
...
Shifted Timestep Schedule. We first construct a schedule of T = 1000 uniformly spaced base values {σi}T −1i=0 $\{ \sigma _ { i } \} _ { i = 0 } ^ { T - 1 }$ over $[ \sigma _ { \mathrm { m i n } } , \sigma _ { \mathrm { m a x } } ] = [ 0 , 1 ]$ . These are then transformed by the shift parameter s [20]:
...
$$
\mathbf {x} _ {\tilde {\sigma}} = \left(1 - \tilde {\sigma} _ {\mathrm{id}}\right) \mathbf {x} _ {0} + \tilde {\sigma} _ {\mathrm{id}} \boldsymbol {\epsilon}, \quad \boldsymbol {\epsilon} \sim \mathcal {N} (0, \mathbf {I}). \tag {15}
$$
