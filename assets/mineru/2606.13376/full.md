...
Yang Zhou13∗† , Ziheng Wang23∗† , Yuqin Lu13∗† , Haofeng Liu3‡ , Jun Liang3‡ , Shengfeng He4 , Jing Li3

1South China University of Technology 2Columbia University 3Orange Team, Moku Lab, HUJING Digital Media & Entertainment Group 4Singapore Management University
...
Project Homepage: https://orange-3dv-team.github.io/MoVerse/

Date: June 18, 2026

![](images/e17bb6a626dee9b99c61c37d7c44df1c6ea1512739428a6bd108f8b44045f4c2.jpg)

<details>
<summary>natural_image</summary>

Illustration of a thatched cottage in a green field with trees and hills, displayed within a game controller interface (no text or symbols on the scene itself)
</details>

∗ Work done during an internship at Orange Team, Moku Lab, HUJING Digital Media & Entertainment Group.  
† Equal contribution.  
‡ Project leader.
...
![](images/f8dfd543981e634aebc2b6a59e10ca86377cc387d9a2be51a946a8f9f8c9a2a7.jpg)
...
```mermaid
graph TD
  A["(a) Panoramic Generation"] --> B["Pano DiT"]
  B --> C["User Controlled Interactive Render"]
  D["(b) Gaussian Generation & Rendering"] --> E["Pano Gaussian Generator"]
  E --> F["User Controlled Interactive Render"]
  G["(c) Autoregressive Video Refinement"] --> H["VAE"]
  H --> I["KV Cache"]
  I --> J["Self-Attention"]
  I --> K["Cross-Attention"]
  I --> L["FFN"]
...
1. Stage I: Panoramic Generation $( I \to P )$ . The input image is expanded into a gravity-aligned $3 6 0 ^ { \circ }$ equirectangular panorama P . This step closes the missing field of view before any 3D lifting, providing structurally complete observations for every viewing direction and eliminating the need for the subsequent 3D stage to hallucinate large unobserved regions.  
2. Stage II: Gaussian Scene Generation $( P \to { \mathcal { G } } )$ . The panorama is lifted into a panoramic 3D Gaussian scene $\mathcal { G } = \{ ( \mu _ { k } , \Sigma _ { k } , \alpha _ { k } , c _ { k } ) \} _ { k = 1 } ^ { K }$ , where each Gaussian is parameterized by its center $\mu _ { k }$ , covariance $\Sigma _ { k }$ , opacity $\alpha _ { k }$ , and appearance $c _ { k }$ . Unlike point-cloud or mesh representations, 3D Gaussians are directly splatting-renderable at real-time frame rates, yielding a dense, camera-controllable RGB conditioning video $\hat { V } _ { 1 : T } = \{ \hat { V } _ { t } \} _ { t = 1 } ^ { T }$ 1 that is substantially more informative than sparse point-cloud projections, but still

![](images/409a47f502658aa0004265812fcf43a8e0a1ec8f17d086ae754f8193eb02ff85.jpg)
...
```mermaid
graph TD
  A["Perspective sampling"] --> B["Gravity-Aligned"]
  B --> C["Perspective sampling"]
  C --> D["Warping"]
  D --> E["Canonical Viewing Space"]
  E --> F["Differentiable Auto-Leveling"]
  F --> G["Geometric Encoder"]
  G --> H["Dense Correspondence"]
  H --> I["Soft-argmin Solver"]
  I --> J["Rigid Correspondence"]
  J --> K["Vertical Embedding Structure"]
    
  L["Text Encoder"] --> M["Add Noise"]
  M --> N["Horizontal Shift"]
  N --> O["DiT LoRA"]
  O --> P["Diffusion Loss"]
  P --> Q["Horizontal Shift"]
  Q --> R["Siamese Consistency Loss"]
    
  S["Text Encoder"] --> T["Add Noise"]
  T --> U["Horizontal Shift"]
  U --> V["DiT LoRA"]
  V --> W["Diffusion Loss"]
  W --> X["Horizontal Shift"]
  X --> Y["Siamese Consistency Loss"]
    
  Z["Text Encoder"] --> AA["Add Noise"]
  AA --> AB["Horizontal Shift"]
  AB --> AC["DiT LoRA"]
  AC --> AD["Diffusion Loss"]
```
</details>
...
$$
\hat {\epsilon} = \epsilon_ {\theta} (z _ {t} \oplus M \oplus z _ {\text { ref }}, t, \tau (y)), \tag {3}
$$
...
$$
\mathcal {L} _ {\mathrm{LDM}} = \mathbb {E} _ {z _ {0}, \epsilon , t} \left[ \| \epsilon - \epsilon_ {\theta} (z _ {t} \oplus M \oplus z _ {\text { ref }}, t, \tau (y)) \| _ {2} ^ {2} \right]. \tag {4}
$$
...
$$
z _ {\text { can }} = \mathcal {T} _ {\text { align }} (z _ {\text { ref }}), \tag {5}
$$
...
$$
P _ {\text { dense }} \in \mathbb {R} ^ {H _ {I} \times W _ {I} \times 2}, \tag {6}
$$

which represents pixel-wise shifts toward a gravity-leveled reference. Instead of directly using this dense field as a free-form spatial warp, the correspondences are passed through a rigid transformation bottleneck implemented by a differentiable soft-argmin solver. The solver searches for a low-dimensional rigid spherical transformation $\hat { R }$ whose reprojection best explains the dense correspondences, including the gravity-alignment rotation needed to remove pitch and roll under the canonical yaw convention. The actual canonical warp is then performed by spherical grid sampling:

$$
z _ {\text { can }} = \mathcal {W} (z _ {\text { ref }}, \hat {R}), \tag {7}
$$
...
$$
\mathcal {L} _ {\mathrm{flow}} = \operatorname{Smooth} _ {L _ {1}} (P _ {\mathrm{dense}}, P _ {\mathrm{GT}}), \tag {8}
$$
...
We further encourage shift-equivariant diffusion generation. Let $\operatorname { R o l l } _ { \delta } ( \cdot )$ denote a horizontal circular shift by offset δ. For the canonical diffusion input ${ \cal X } = z _ { t } \oplus { \cal M } \oplus z _ { \mathrm { c a n } } .$ the base and shifted branches are

$$
\hat {\epsilon} _ {\text { base }} = \epsilon_ {\theta} (X, t, \tau (y)), \tag {9}
$$

$$
\hat {\epsilon} _ {\text { shifted }} = \epsilon_ {\theta} (\operatorname{Roll} _ {\delta} (X), t, \tau (y)). \tag {10}
$$

The shift-consistency objective is

$$
\mathcal {L} _ {\mathrm{shift}} = \mathbb {E} _ {z _ {0}, \epsilon , t, \delta} \left[ \| \mathrm{Roll} _ {\delta} (\hat {\epsilon} _ {\mathrm{base}}) - \hat {\epsilon} _ {\mathrm{shifted}} \| _ {2} ^ {2} \right]. \tag {11}
$$
...
$$
P _ {\text { centered }} = \operatorname{Roll} (P _ {\mathrm{ERP}}, - \psi_ {\text { input }}). \tag {12}
$$

Canonicalization  
![](images/bb9125f2333781d06f057f170e242037b5b7fa6da86ea58aee7d0cba83790db6.jpg)
...
$$
\mathcal {L} _ {\text { stage1 }} = \mathcal {L} _ {\mathrm{LDM}} + \lambda_ {\text { shift }} \mathcal {L} _ {\text { shift }} + \lambda_ {\text { flow }} \mathcal {L} _ {\text { flow }}. \tag {13}
$$
...
$$
\theta = \left(\frac {u}{W} - \frac {1}{2}\right) 2 \pi , \quad \phi = \left(\frac {1}{2} - \frac {v}{H}\right) \pi , \tag {14}
$$

and hence to a unit viewing direction

$$
\mathbf {d} (\theta , \phi) = \bigl (\cos \phi \sin \theta , - \sin \phi , \cos \phi \cos \theta \bigr). \tag {15}
$$

Given an estimated panorama depth map $D ,$ we instantiate Gaussians on a strided ERP grid $\Omega ^ { \prime } = \{ ( u _ { k } , v _ { k } ) \} _ { k = 1 } ^ { K }$ with resolution $H ^ { \prime } \times W ^ { \prime }$ , s o $K = H ^ { \prime } W ^ { \prime }$ . For each grid location, let

$$
\theta_ {k} = \theta (u _ {k}), \quad \phi_ {k} = \phi (v _ {k}), \quad \mathbf {d} _ {k} = \mathbf {d} (\theta_ {k}, \phi_ {k}), \quad D _ {k} = D (u _ {k}, v _ {k}). \tag {16}
$$

![](images/439893ab816692eb2d7cfda4a3253963421a96121a191db00a3cdd0145795d2d.jpg)
...
```mermaid
graph TD
  A["Panorama"] --> B["Initializer"]
  B --> C["Initial Gaussians"]
  C --> D["Final Gaussians"]
  D --> E["Attribute Delta"]
  E --> F["Attribute Predictor"]
  F --> A
```
</details>
...
$$
\mu_ {k} = D _ {k} \mathbf {d} _ {k}. \tag {17}
$$
...
$$
s _ {k} \propto D _ {k} \cos \phi_ {k}, \tag {18}
$$
...
$$
\mathcal {G} = \left\{\left(\mu_ {k}, s _ {k}, q _ {k}, c _ {k}, \alpha_ {k}\right) \right\} _ {k = 1} ^ {K}, \tag {19}
$$
...
We compose all predicted Gaussian quantities as residuals around the initialized scaffold. The most important case is the Gaussian center, because ERP panoramas use an angular image domain rather than a planar one. In a pinhole camera, image-plane offsets are naturally expressed in normalized device coordinates. In an ERP panorama, horizontal motion changes longitude and vertical motion changes latitude. For each primitive $k ,$ we therefore predict position offsets $( \Delta \theta _ { k } , \Delta \phi _ { k } , \Delta z _ { k } )$ and apply them in angular–inverse-depth space before spherical back-projection:

$$
\theta_ {k} ^ {\prime} = \theta_ {k} + \lambda_ {x y} \Delta \theta_ {k}, \qquad \phi_ {k} ^ {\prime} = \mathrm{clip} (\phi_ {k} + \lambda_ {x y} \Delta \phi_ {k}, - \pi / 2 + \epsilon , \pi / 2 - \epsilon), \qquad D _ {k} ^ {\prime} = \frac {1}{\mathrm{softplus} (\rho_ {k} + \lambda_ {z} \Delta z _ {k}) + \epsilon}, (2 1)
$$
...
$$
\mu_ {k} = D _ {k} ^ {\prime} \mathbf {d} (\theta_ {k} ^ {\prime}, \phi_ {k} ^ {\prime}). \tag {22}
$$
...
$$
\mathcal {L} _ {\mathrm{tgt}} = \lambda_ {1} \| R (\mathcal {G}) - I ^ {t g t} \| _ {1} + \lambda_ {\mathrm{lpips}} \operatorname{LPIPS} (R (\mathcal {G}), I ^ {t g t}) + \lambda_ {\mathrm{ssim}} \bigl (1 - \operatorname{SSIM} (R (\mathcal {G}), I ^ {t g t}) \bigr). \tag {23}
$$
...
$$
\mathcal {L} _ {\text { depth }} = \| D _ {\text { pred }} ^ {- 1} - D _ {\text { gt }} ^ {- 1} \| _ {1}. \tag {24}
$$

We further regularize the angular–depth position residuals,

$$
\mathcal {L} _ {\Delta} = \operatorname{mean} \bigl (\max (| \Delta_ {\theta \phi z} | - \delta , 0) \bigr), \tag {25}
$$
...
$$
\mathcal {L} _ {\mathrm{GS}} = \mathcal {L} _ {\mathrm{tgt}} + \lambda_ {\mathrm{src}} \mathcal {L} _ {\mathrm{src}} + \lambda_ {\mathrm{depth}} \mathcal {L} _ {\mathrm{depth}} + \lambda_ {\Delta} \mathcal {L} _ {\Delta}. \tag {26}
$$
...
$$
\hat {V} _ {t} = \operatorname{Render} (\mathcal {G}, \pi_ {t}), \quad \hat {V} _ {1: T} = \{\hat {V} _ {t} \} _ {t = 1} ^ {T}. \tag {27}
$$
...
![](images/77160372621cefc1de23f213e645f0e10d213e5bc4e8155bb88d2868695785d5.jpg)
...
```mermaid
graph TD
  A["Denoising loop (T=2 steps)"] --> B["KA"]
  B --> C["KV cache history (MemRoPE)"]
  C --> D["Causal DiT (noisy → clean)"]
  D --> E["Layer 1"]
  D --> F["Layer 2"]
  D --> G["..."]
  D --> H["Layer N"]
  I["Clean rerun → next-block KV"] --> J["Layer 1"]
  I --> K["Layer 2"]
  I --> L["..."]
  I --> M["Layer N"]
  N["VAE"] --> O["Image of residential buildings"]
  P["Height Concat"] --> Q["× T steps (re-noise to next o, consistency inference)"] --> D
  Q --> R["Layer 1"]
  Q --> S["Layer 2"]
  Q --> T["..."]
  Q --> U["Layer N"]
  V["Random Latent Noise"] --> W["Input Latent"]
  X["Rendered Frames Latent"] --> Y["Input Latent"]
  Z["Green Block"] --> AA["+"]
  AB["Gray Block"] --> AC["+"]
  AD["Red Block"] --> AE["+"]
  AF["Blue Block"] --> AG["+"]
  AH["White Block"] --> AI["+"]
  AJ["Light Gray Block"] --> AK["+"]
  AL["Light Grey Block"] --> AM["+"]
  AN["Light Pink Block"] --> AO["+"]
  AP["Light Red Block"] --> AQ["+"]
  AR["Light Pink Block"] --> AS["+"]
  AT["Light Pink Block"] --> AU["+"]
  AV["Light Pink Block"] --> AW["+"]
  AX["Light Pink Block"] --> AY["+"]
  AZ["Light Pink Block"] --> BA["+"]
  BB["Light Pink Block"] --> BC["+"]
  BD["Light Pink Block"] --> BE["+"]
  BF["Light Pink Block"] --> BG["+"]
  BH["Light Pink Block"] --> BI["+"]
  BJ["Light Pink Block"] --> BK["+"]
  BL["Light Pink Block"] --> BM["+"]
  BN["Light Pink Block"] --> BO["+"]
  BP["Light Pink Block"] --> BQ["+"]
  BR["Light Pink Block"] --> BS["+"]
  BT["Light Pink Block"] --> BU["+"]
  BV["Light Pink Block"] --> BW["+"]
  BX["Light Pink Block"] --> BY["+"]
  BZ["Light Pink Block"] --> BQ
  CA["Light Pink Block"] --> CA
  CB["Light Pink Block"] --> CB
  CC["Light Pink Block"] --> CC
  DD["Light Pink Block"] --> DD
  DX["Light Pink Block"] --> DX
  DB["X"] --> DC
  DBX["X"] --> DC
```
</details>
...
$$
z _ {t} = (1 - \sigma_ {t}) z + \sigma_ {t} \epsilon , \quad \epsilon \sim \mathcal {N} (0, I). \tag {29}
$$

We concatenate the noisy target tokens and conditioning tokens as

$$
\tilde {z} = \left[ z _ {t} \| c \right]. \tag {30}
$$

The two halves use shared rotary positional indices: every conditioning token receives the same spatial– temporal position code as its aligned target token. This shared-RoPE design makes the condition and target collide at the same location in the transformer coordinate system, encouraging the model to treat c as a dense aligned rendering condition rather than as a separate video placed at an offset position.

The teacher is trained with a flow-matching objective, evaluated only on the target half of the concatenated tokens:

$$
\mathcal {L} _ {\text { teacher }} = \mathbb {E} _ {z, c, \tau , \sigma_ {t}, \epsilon} \left[ \| v _ {\theta} (\tilde {z}, \tau , t) - (\epsilon - z) \| _ {2} ^ {2} \right], \tag {31}
$$
...
trajectory-aligned geometric proxy → explicit Gaussian render → panorama-conditioned Gaussian render. (32)
...
![](images/4b50f020a1780486a281dd0a3cb9c7dfad7c1610a41c5417c66113168c7e8e1d.jpg)  
...
![](images/1936ff936af92fc962f7c49dbd09d6c10bade9b56eaf817de034fd5163fca31d.jpg)  
...
![](images/fd3c6d06c7f387ec6da72fec44145049d6664df2388ce934a349a7fb557ad822.jpg)

<details>
<summary>natural_image</summary>

Collage of interior and exterior views of a modern luxury hotel with traditional Chinese architecture, featuring dining areas, water features, and a pool (no visible text or signage)
</details>
...
![](images/af5828896e2e693f7cf3638a00bbbef30f9bb0c8d1d5318967e28ccbc64956a9.jpg)

<details>
<summary>natural_image</summary>

Collage of futuristic interior views including a planetarium, spacecraft, and cosmic structures with glowing elements (no visible text or symbols)
</details>
